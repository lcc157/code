...
#from .models.att_MobileNetV2 import MobileNetV2, InvertedResidual
#from .models.att_MobileNetV2 import MobileNetV2, InvertedResidual
from .models.MobileNetV3 import MobileNetV3,InvertedResidual,InvertedResidual1
import time
...
class BaseModel(nn.Module):
    ...
    def load_best_weights1(self, directory):
        self.load_state_dict(torch.load(directory +'/weights_best.pth'))
    ...
    def load_epoch_checkpoint(self, directory, epoch='13'):
    ...
    def load_checkpoint(self, file):
        ...
        #     f.write(str(chkpnt))
        self.load_state_dict(chkpnt['model_state_dict'])
        # with open('./last_model_state.txt', 'w') as f:
        #     f.write(str(self.load_state_dict))
...
default_cnn_cfg = {
    'width_mult': 1.,  'input_channel': 16,
    'last_channel': 255} #1. 576 1280 1024
...
class UNISAL(BaseModel, utils.KwConfigClass):
    ...
    def __init__(self,
                # rnn_input_channels=256, rnn_hidden_channels=256,
                 ...
                 #res_rnn=True,
                 bypass_rnn=True,
                 drop_probs=(0.0, 0.5, 0.5), #default 0.6
        ...
        self.cnn = MobileNetV3(**self.cnn_cfg)
        # input_size=torch.randn(1, 3, 96, 128)
        # flops, params = profile(self.cnn, inputs=(input_size,))
        # print('Total params: %.2fM' % (params/1000000.0))
        # print('Total flops: %.2fM' % (flops/1000000.0))

        # Initialize Post-CNN module with optional dropout
        post_cnn = [
            ('inv_res', InvertedResidual1(
                self.cnn.out_channels + n_gaussians,
                256, 
                1, 1, bn_momentum=bn_momentum,
            ))
        ]
    ...
    def forward(self, x, target_size=None, h0=None, return_hidden=False,
        ...
        t4=time.time()-to
        #print("CNN && Gaussian：{:.3}s".format(dt),"RNN{:.3}s".format(t2),"Decoder{:.3}s".format(t4),"out_shape",im_feat.shape[-2:])
