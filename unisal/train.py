...
from . import utils
#from . import model
#from . import model_ghost as model
from . import model_v3 as model
from . import data
cv2.setNumThreads(0)
...
class Trainer(utils.KwConfigClass):
    ...
    def fit_phase(self):
        ...
        for src in sources_eval:
            ...
            if src == "SALICON" and self.phase == 'valid' and\
    ...
    def fit_sample(self, sample, grad_clip=None, **kwargs):
        ...
        with torch.set_grad_enabled(self.phase == 'train'):
            ...
            #BCE = torch.nn.BCELoss() #BCE LOSS
            #loss2=BCE(torch.sigmoid(pred_seq), sal)
            #losses.append(utils.kld_loss(pred_seq, sal_seq))
            #focaloss=utils.FocalLoss(pred_seq, sal)
           
            #loss=loss1+0.001*l2_regularization+focaloss
    ...
    def run_inference(self, source, vid_nr, dataset=None, phase=None,
        ...
        results_size = (1, n_images, 1, model_kwargs['target_size'][0],model_kwargs['target_size'][1])
    ...
    def generate_predictions_from_path1(
        ...
        images_path = os.path.join(folder_path, 'images/test')
        torch.cuda.empty_cache()
    ...
    def generate_predictions_from_path(
        ...
        if source in ('MIT1003', 'MIT300'):
            ...
            # print(params)
        #images_path = folder_path / 'images'/'examples'
        print(folder_path)
        images_path = folder_path /'test_imgs'
        #images_path = folder_path / 'images'/'val'
        ...
        if load_weights:
            ...
            try:
                ...
                self.model.load_last_chkpnt(self.train_dir)
                print('Last checkpoint loaded')
        # Measure the average time to process single frames on the CPU
        # for name, param in self.model.named_parameters():
        ...
        with torch.no_grad():
            if is_video:
               ...
               # pred_dir = folder_path / 'bos'
                ...
                dataset = data.FolderImageDataset(images_path)
                pred_dir = folder_path / 'pre_test_imgs2'
                pred_dir.mkdir(exist_ok=True)
                ...
                dt = max(times)
                print("Max single-frame CPU time: " + str(dt) + " s (" + str(1 / dt) + " fps)")     
    ...
    def measure_runtime(self):
        ...
        with torch.no_grad():
            ...
            self.model.eval()
            self.num_workers = 0
            dl = self.get_dataloader('test', 'SALICON')
            sample = next(iter(dl))
            _, x, _ = sample
            x = x.float().cuda()
           # print(x.shape)
    ...
    def scheduler(self):
        ...
        if self._scheduler is None:
            if self.lr_scheduler == 'ExponentialLR':
                ...
                #余弦退火
                self._scheduler=torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, 32, eta_min=0, last_epoch=-1)
                #ReduceLROnPlateau
                #self._scheduler=torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer,'min',factor=0.5,patience=4, verbose=True)
