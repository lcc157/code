...
class GhostNet(nn.Module):
    def __init__(self, width=1., dropout=0.2,last_channel=1280,input_channel=16):
        ...
        stages = [conv_3x3_bn(3, output_channel, 2)]
        ...
        for k, exp_size, c, se_ratio, s in interverted_residual_setting:
            output_channel = _make_divisible(c * width, 4)
            hidden_channel = _make_divisible(exp_size * width, 4)
            stages.append(block(input_channel, hidden_channel, output_channel, k,s,
                                    se_ratio=se_ratio))
            input_channel = output_channel
        #stages.append(nn.Sequential(*stages))

        output_channel = 1280
        # building last several layers
        if self.last_channel is not None:
            output_channel = int(self.last_channel * width) \
                if width > 1.0 else self.last_channel
            stages.append(nn.Sequential(ConvBnAct(input_channel, output_channel, 1)))
        self.blocks = nn.Sequential(*stages)
        self.out_channels = output_channel
        # self.feat_1x_channels = int(
        #     interverted_residual_setting[-1][1] * width)
        self.feat_2x_channels = 112
        self.feat_4x_channels = 40
        # self.feat_8x_channels = int(
        #     interverted_residual_setting[-5][1] * width)

    def forward(self, x):
        # x = self.features(x)
        feat_2x, feat_4x, feat_8x = None, None, None
        for idx, module in enumerate(self.blocks._modules.values()):
           # print(idx)
            x = module(x)
           # print(x.shape)
            if idx == 5:
