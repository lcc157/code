...
from .att_module import eca_layer , cbam_layer
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio, omit_stride=False,ksize=3,
        ...
        if expand_ratio == 1:
            ...
            #modules.append(eca_layer(oup, self.ksize)) #lc eca-attention
            #modules.append(cbam_layer(oup))
            if self.dropout > 0:
           ...
           # modules.append(cbam_layer(oup))
            ...
            #modules.append(eca_layer(oup, self.ksize))
            self.conv = nn.Sequential(*modules)
    ...
    def __init__(self, widen_factor=1., pretrained=True,
        ...
        for t, c, n, s in interverted_residual_setting:
            ...
            for i in range(n):
                if c < 96:
                    ...
                    ksize = 3
                o_stride =True if i == 0 else False
                stride = s if i == 0 else 1
                #print(i,o_stride,stride)
                self.features.append(block(input_channel, output_channel, stride, expand_ratio=t,omit_stride=o_stride,ksize=ksize))
                input_channel = output_channel
        ...
        if self.pretrained:
            state_dict = torch.load(
                Path(__file__).resolve().parent / 'weights/eca_mobilenetv2.pth.tar')
